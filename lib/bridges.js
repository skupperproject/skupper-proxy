...
function AmqpToHttpBridge(address, host, port) {
    ...
    this.connection.on('connection_open', this.connected.bind(this));
...
AmqpToHttpBridge.prototype.connected = function (context) {
    console.log('Connected to router %s:%s', this.host, this.port);
};

AmqpToHttpBridge.prototype.disconnected = function (context) {
    console.log('Disconnected from router %s:%s', this.host, this.port);
};
...
function AmqpToTcpBridge(address, host, port) {
    ...
    this.mgmt_client.request({application_properties:props,body:definition}).then(function (result) {
        if (result.application_properties.statusCode >= 200 && result.application_properties.statusCode < 300) {
            ...
            self.connection.on('receiver_open', self.on_receiver_open.bind(self));
        } else {
