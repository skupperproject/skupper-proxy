...
IngressStream.prototype.do_send = function () {
    ...
    var msg = {correlation_id: this.stream.id, application_properties: {'ended': this.request_ended}, body: data};
...
Egress.prototype.on_request_message = function (context) {
    ...
    if (ended) {
        ...
        new EgressStream(this.connection.open_sender(context.message.reply_to), request, this.reply_to, context.message.correlation_id);
    } else {
        ...
        var egress_stream = new EgressStream(this.connection.open_sender(context.message.reply_to), request, this.reply_to, context.message.correlation_id);
...
function EgressStream(sender, request, reply_to, correlation_id) {
...
EgressStream.prototype.on_end = function (data) {
    ...
    this.request_ended = true;
    if (data) this.responses.push(data);
...
EgressStream.prototype.send_response = function (data, ended) {
    ...
    var response = {correlation_id: this.correlation_id, reply_to:this.reply_to, application_properties:{ended:ended}, body:data};
    if (this.headers) {
