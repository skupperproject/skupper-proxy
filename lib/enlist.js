...
function not_already_enlisted (service) {
...
Enlist.prototype.services_updated = function (services) {
    ...
    this.update_services(services.filter(has_proxy_annotation).filter(not_already_enlisted).map(service_to_service_record));
};
