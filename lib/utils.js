...
//from https://stackoverflow.com/a/35560645/6620104
const ipv6_template = /^:+(ffff)?:(?!0)(?!.*\.$)((1?\d?\d|25[0-5]|2[0-4]\d)(\.|$)){4}$/
module.exports.client_ip = function (socket) {
    var ip = socket.remoteAddress;
    // check for IPV4-mapped IPV6 address
    if (ipv6_template.test(ip)) {
        return ip.replace(/^.*:/, '');
    } else {
        return ip;
    }
