...
function get_bridging_functions(protocol, config) {
    if (protocol === 'http') {
        if (config.aggregate) {
            return {
                ingress: function (port, address) {
                    return multicast.http_to_amqp(port, address, config.aggregate)
                },
                egress: multicast.amqp_to_http
            };
        } else if (config.eventchannel) {
            return {
                ingress: eventchannel.http_to_amqp,
                egress: eventchannel.amqp_to_http
            };
        } else {
            return {
...
function Proxy(config) {
    var bridging = get_bridging_functions(config.protocol, config);
    if (bridging) {
